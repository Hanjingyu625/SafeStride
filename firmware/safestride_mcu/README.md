...
60, FF bias는 30이며 Hall 보정만으로 출력이 0이 되지 않도록 PWM 1 count의
최종 하한을 둔다. Hall 무펄스·implausible 결과와 속도 초과는 진단으로만
남고 PWM 차단이나 BRAKE를 만들지 않는다. 어느 한쪽 압력 손잡이라도 잡혀 있으면
dead-man이 유지되고 양쪽을 모두 놓았을 때만 0.6초 감속 후 정지한다. command
watchdog과 motor-driver critical fault 처리는 유지한다.
