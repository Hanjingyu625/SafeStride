...
size_t cobsDecode(
  ...
  while (read_index < length) {
    ...
    if (write_index + copy_count > capacity) {
      return 0U;
    }
...
bool sequenceIsNewer(uint16_t candidate, uint16_t previous) {
  const uint16_t difference =
      static_cast<uint16_t>(candidate - previous);
...
bool sendFrame(
  ...
  writeU16(
      raw + checked_length,
      crc16CcittFalse(raw, checked_length));
  const size_t raw_length = checked_length + CRC_SIZE;
  ...
  const size_t written = stream.write(encoded, encoded_length);
  const size_t delimiter_written = stream.write(static_cast<uint8_t>(0U));
  return written == encoded_length && delimiter_written == 1U;
}
