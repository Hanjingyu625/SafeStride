...
void DriveController::update(
  ...
  // Hall speed is advisory. It may trim PWM but must never request BRAKE.
  // Keep the legacy monitor behind an explicit disabled policy switch so a
  // future tuning change cannot silently restore a Hall-triggered stop.
  ...
  if (cfg::ENABLE_HALL_SPEED_BRAKE) {
    ...
    if (speed_brake_dwell_us_ >= cfg::SPEED_BRAKE_DWELL_US) {
      speed_brake_ = true;
    }
    if (speed_brake_ &&
        ((speed_valid_ &&
          observed_speed < requested +
              cfg::SPEED_BRAKE_RELEASE_MARGIN_MRAD_S &&
  ...
  if (cap > 0.0F && fabsf(target) >= 20.0F &&
      fabsf(output) < static_cast<float>(cfg::MOTOR_MIN_ACTIVE_PWM)) {
    const float floor = fminf(
        cap, static_cast<float>(cfg::MOTOR_MIN_ACTIVE_PWM));
    output = target > 0.0F ? floor : -floor;
  }
  writeMotor(output);
}
