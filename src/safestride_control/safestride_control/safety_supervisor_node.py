...
class SafetySupervisor(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter('minimum_auxiliary_speed_scale', 0.25)
        ...
        self._minimum_auxiliary_scale = finite_parameter(
            'minimum_auxiliary_speed_scale',
            self.get_parameter('minimum_auxiliary_speed_scale').value,
            minimum=0.0,
            maximum=1.0,
    ...
    def _status_reasons(self, now: float) -> List[str]:
        ...
        if status.fault_bits & CRITICAL_MCU_FAULT_MASK:
    ...
    def _terrain_reasons(self, now: float) -> List[str]:
        # TOF and Terrain link health are published by device_health_monitor.
        # They are advisory and never enter the motor-stop reason list.
        del now
        return []
    ...
    def _range_scale(self, distance: float) -> float:
        if distance <= self._stop_distance:
            return self._minimum_auxiliary_scale
        ...
        return max(self._minimum_auxiliary_scale, scale)
    ...
    def _surface_state(
        ...
        if not valid:
            return [], 1.0, surface.classification, confidence
        return (
            [],
            max(self._minimum_auxiliary_scale, scale),
