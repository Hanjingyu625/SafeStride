    ...
    def update(
        ...
        *,
        ...
        if (
            ...
            self.reset()
            # The IMU is advisory except for a positively observed steep
            # downhill. Missing or malformed samples therefore restore the
            # neutral scale instead of stopping motion.
...
class SlopeBrakePolicy:
    ...
    def update(self, pitch_rad, now_s, enabled=True):
        ...
        if not valid:
            # Sensor loss is diagnostic-only. Never retain or enter BRAKE
            # without a current, finite steep-downhill observation.
            self.braking = False
            self.clear_since = None
        elif angle <= -self.enter:
            self.braking = True
            self.clear_since = None
        elif self.braking:
