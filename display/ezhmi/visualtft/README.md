...
3. 프로젝트를 컴파일한다.
4. 가상 화면에서 인트로, 상태 화면, 한글 출력을 확인한다.
...
- 화면 1의 기능 컨트롤 ID 1~16은 `../controls.csv` 및 Lua와 연결되므로 바꾸지 않는다.
- ID 17~21은 배경 카드와 상태 바를 위한 장식용 컨트롤이다.
- Modbus 변수 주소와 기본값은 `../registers.csv`가 기준이다.
- 편집 가능한 Lua 원본은 UTF-8인 `../safestride.lua`다.
- VisualTFT 3.0이 저장한 `main.lua`는 Windows-949 인코딩 사본이다. Lua를 바꿀 때는
  두 파일의 내용이 같도록 동기화하고 컴파일한다.
- DEV 버튼(ID 11)은 동작과 송신 데이터가 없으며 Lua에서도 비활성화한다.

저장소 테스트는 XML, 컨트롤 ID와 좌표, Modbus 변수 및 두 Lua 사본의 일치를
검사한다. 실제 VisualTFT 컴파일, 한글 렌더링과 LCD 다운로드는 Windows 도구와
