...
int main() {
  ...
  run(slew,100,696,752298UL);
  assert(g_motor_pwm <= 11);
  int before=g_motor_pwm;
  HallSample h={2000,752298UL,0};
  slew.update(5000,h,h,0,true,true,1160,true);
  assert(g_motor_pwm<=before);
  run(slew,1,0,752298UL); assert(g_motor_pwm==0);
  ...
  assert(g_motor_pwm>0);  // Hall stall is diagnostic-only.

  // Hall overspeed may reduce PWM but can never trigger BRAKE.
  DriveController fast; fast.begin(); primeFeedback(fast);
  run(fast,800,696,752298UL);
  run(fast,45,696,250000UL);
  assert(g_motor_pwm>0 && !fast.braking());
  run(fast,900,696,752298UL);
