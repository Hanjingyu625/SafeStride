...
static void ack(bool corrupt=false) {
  uint8_t bytes[8]={1,0x10,0,0,0,hmi::WORDS,0,0};
...
static void verifyScenario(const uint16_t* expected) {
  ...
  uint8_t frame[7+hmi::WORDS*2+2];
  assert(hmi::buildWrite(frame,1,0,state.words)==sizeof(frame));
  assert(frame[0]==1 && frame[1]==0x10);
  assert(frame[2]==0 && frame[3]==0 && frame[4]==0 && frame[5]==hmi::WORDS);
  assert(frame[6]==hmi::WORDS*2);
  for (size_t i=0; i<hmi::WORDS; ++i) {
...
int main() {
  ...
  const uint16_t scenarios[][hmi::WORDS] = {
    {3,1,63,125,3,3,15,42,0,0,0,2,0,0,1,83},
    {3,1,63,85,3,2,65535,35,85,0,0,2,0,0,1,3},
    {3,1,63,70,3,4,9,21,65466,0,0,2,0,0,1,67},
    {3,1,63,20,3,2,5,15,0,0,0,2,1,0,1,67},
    {3,1,63,0,3,2,65535,15,0,3,1,2,0,0,1,3},
    {3,1,0,65535,0,0,65535,65535,0,5,0,0,0,0,1,0},
    // WET surface, 24/31 confidence, plus armed+deadman.
    {3,1,63,125,3,3,15,42,0,0,0,2,0,0,1,50051},
  };
  for (size_t i=0; i<sizeof(scenarios)/sizeof(scenarios[0]); ++i)
    verifyScenario(scenarios[i]);

  hmi::State state;
  uint16_t words[hmi::WORDS]={3,1,63,125,3,3,6,42,65526,3,1,2,0,0,1,19};
  uint8_t payload[hmi::WORDS*2]; pack(words,payload);
  assert(state.accept(payload,sizeof(payload),65535,100));
  assert(!state.accept(payload,sizeof(payload),65535,101));
  assert(state.accept(payload,sizeof(payload),0,102)); // sequence wraps
  assert(!state.accept(payload,sizeof(payload)-1,1,103));
  payload[0]=1; assert(!state.accept(payload,sizeof(payload),1,103)); payload[0]=3;

  hmi::State invalid_surface;
  uint16_t invalid_words[hmi::WORDS]={3,1,0,65535,0,0,65535,65535,0,5,0,0,0,0,1,0x0780};
  pack(invalid_words,payload); // surface-valid with reserved class 7
  assert(!invalid_surface.accept(payload,sizeof(payload),1,100));
  invalid_words[15]=0x0100; // surface bits without surface-valid
  pack(invalid_words,payload);
  assert(!invalid_surface.accept(payload,sizeof(payload),1,100));
  ...
  assert(state.accept(payload,sizeof(payload),5,703));
  assert(state.words[hmi::HAZARD]==0);

  words[hmi::BASE_WORDS]=0x5355;  // "SU", first packed location pair.
  uint8_t frame[7+hmi::WORDS*2+2];
  assert(hmi::buildWrite(frame,1,0,words)==sizeof(frame));
  assert(frame[0]==1 && frame[1]==0x10 && frame[6]==hmi::WORDS*2);
  assert(frame[7]==0 && frame[8]==3);
  assert(frame[23]==255 && frame[24]==246); // signed pitch -1.0 deg
  assert(frame[39]=='S' && frame[40]=='U');
  assert(hmi::modbusCrc(frame,sizeof(frame))==0);
  ...
  lcd.tick(3000,state);
  assert(hmi_uart_test::tx().size()==sizeof(frame));
