...
9는 MPU6050이다. Drive bit 10은 Hall/보조센서가 모터를 정지시키지 않고 어느 한쪽
압력만 유지돼도 dead-man이 성립하는 advisory-sensor 정책을 뜻한다. 새 bridge는
구형 정지 정책 펌웨어와 섞이지 않도록 Drive bit 10을 arm 전에 요구한다. E-stop
bit 5는 현재 광고하지 않는다.
...
Drive Uno는 session, 최신 command TTL, 유효한 압력 telemetry, 적어도 한쪽 손,
critical motor-driver fault 없음 및 명시적 enable을 모두 만족할 때만 PWM을
허용한다. Hall/TOF/카메라/GPS 등 보조센서의 invalid·stale·plausibility 결과는
진단과 제한에만 사용하며 모터 정지를 만들지 않는다. Hall fault bit는 계속
telemetry에 남지만 advisory다. 모터 정지는 명시적 0/BRAKE, 양쪽 손 해제,
Drive 통신/watchdog, motor-driver fault 또는 실제로 관측된 급내리막에서만 난다.
