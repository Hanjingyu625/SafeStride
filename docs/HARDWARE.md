...
| 핀 | 연결 |
|---:|---|
| A3 | 왼쪽 휠 WSH135 `OUT`, 아날로그 기준점/히스테리시스 감지, 자석 12개 |
| D5/D6/D8 | 공통 모터드라이버 PWM/IN1/IN2 |
| A2/A1 | 왼쪽/오른쪽 압력센서, threshold 80 |
| A0/A4 | 선택적 전류센서 예약, 현재 비활성 |
...
WSH135는 5 V 구동 선형 아날로그 센서다. 마킹이 보이는 평평한 면을 정면으로
보고 다리를 아래로 했을 때 왼쪽부터 `VDD -> 5V`, `GND -> GND`,
`OUT -> A3`로 연결한다. 출력에는 풀업/풀다운 저항을 달지 않는다. 필요하면
`OUT-GND` 사이에 0.01~0.1 uF 커패시터를 센서 바로 옆에 단다. 부팅 중 학습되는
약 2.5 V 무자계 기준을 오염시키지 않도록 전원을 넣을 때 자석을 떼어 둔다.
바퀴 지름은 23 cm, 반지름은 0.115 m로 설정한다.
