...
`30`은 계산 bias다. 유효한 비영 목표에서는 Hall P 보정이 출력을 낮추더라도
최종 PWM을 최소 1 count로 유지해 Hall 판단만으로 BRAKE/0이 되지 않는다. 속도
오차만으로 목표 반대 방향 PWM을 만들지 않는다. Ki=Kd=0이다.
...
| MPU invalid, fault, nonfinite 또는 stale | 중립 경사 배율과 진단 경고, 정지하지 않음 |
| Hall 속도 초과·무펄스·implausible | 진단 경고와 PWM 보정만, 정지하지 않음 |
| 요청속도 0 | BRAKE. 정상 손 해제 ramp는 아래 예외 |

Hall 속도는 pulse-period 원시값과 필터값으로 진단·P 보정에 사용하지만 모터
정지 결정에는 사용하지 않는다. 적어도 한쪽 손이 유지되는 dead-man, 통신
watchdog, critical motor-driver fault, 수동 inhibit를 유지한다. 양쪽 손 해제는
600ms 동안 직전 출력부터 단조 감소시킨다. E-stop 설정은
...
`/diagnostics`에는 정규화 pitch·경사 상태·목표속도·slope FF·MCU PWM·속도
age/valid와 BRAKE 상태를 기록한다. 명시적 급내리막 BRAKE 처리 시 MCU FF/P
표시값과 applied PWM은 0이다. Hall 이상은 장치 진단 WARN으로만 남는다.
...
1. 바퀴를 든 상태에서 LOW/LOW BRAKE와 방향을 확인한다. 평지 장착 pitch의
   offset·sign, 좌우 압력, 5초 Hall age를 함께 확인한다.
2. PWM 30/40/60에서 기동과 회전 유지 조건을 각각 측정하고 `v_nom`과 FF를 보정한다.
3. +5°/-5°/-8°에서 목표속도·slope FF·applied PWM을 기록한다. -10° BRAKE와
   -7° 복구를 시험한다. MPU 분리 시 정지하지 않고 WARN인지 확인한다.
4. Hall 누락·중복·비정상 속도에서 PWM이 0/BRAKE가 되지 않고 진단 WARN만
   발생하는지 확인한다. 양쪽 손 해제 중에는 출력이 증가하지 않아야 한다.
