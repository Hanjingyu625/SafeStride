...
현재 동작: 내리막 5°가 0.5초 지속되어 확정되면 BRAKE/PWM 0.

기준: 현재 저장소의 실행 코드와 설정.
...
현재 supervisor offset `0 rad`, sign `-1`이다. Pitch 양수는 내리막, 음수는 오르막이다.
Terrain bridge가 평지 기준 Pitch `-0.116 rad`(-6.646310°), Roll `-0.059 rad`(-3.380451°)를
한 번 빼서 `/terrain/status`를 발행한다. Foxglove와 HMI, 경사 제어는 이 보정값을 사용한다.
supervisor와 HMI의 추가 offset은 `0`으로 유지해 중복 보정을 피한다.
아래 각도는 모두 보정 후 값이다. `/terrain/imu`는 중력 제거 계산과 일관되도록
센서 좌표계의 기존 자세·가속도를 유지한다. 현재 영점은 평지 화면 한 장 기준이므로
재장착 시에는 정지 상태의 여러 샘플로 `terrain_bridge.attitude` 설정을 다시 보정한다.

아래 표는 기본 Cruise 요청 1.0 m/s와 현재 Pi 설정 기준이다.

| 보정 후 Pitch / 조건 | 목표속도와 출력 동작 |
|---|---|
| 평지 | 목표속도 1.0 m/s |
| 내리막 +5° 미만 (평지에서 진입) | 목표속도 1.0 m/s 유지 |
| 내리막 +5° 이상 0.5초 유지 | 목표 0과 BRAKE를 보내 PWM 즉시 0, IN1/IN2 LOW |
| 내리막 +7° 이상, 5° 확정 전 | 진입 당시 PWM에서 3초 감속을 먼저 시작; 5° 확정 시 BRAKE/PWM 0이 우선 |
| 확정 내리막 후 +3° 이하 | 0.5초 유지 시 평지로 복귀하고 정상 출발 경로 재개 |
| 오르막 -5° 이하 0.5초 유지 | 목표속도 1.0 m/s 유지, 최대 +30 PWM 경사 FF |

오르막 상태는 -3°까지의 히스테리시스로 유지되며, 양의 경사 FF는 -5°보다
완만해지면 0이 된다. 속도 피드백과 손잡이/통신 정지는 표보다 우선한다. ToF는 현재 모니터링 전용이다.
FF 60은 명목속도의 계산값이며 PWM 하한이 아니다. Ki=0을 유지한다.
PWM을 0으로 줄이는 시간과 실제 보행기의 제동거리/정지시간은 별도로 검증한다.

Pi 실행 시 다른 checkout의 설정이 환경변수에 남지 않도록 경로를 명시한다:

```bash
cd /home/pi/SafeStride-pdj1
...
기존 최대 전진속도 1.15 m/s와 가감속 제한은 유지한다. `downhill_speed_scale=1.0`이며
확정 내리막은 속도 배율 대신 `BRAKE`로 처리한다. 5° 확정 전 +7°가 먼저 감지되면
기존 3초 PWM 감속을 시작하지만, 0.5초 뒤 내리막이 확정되면 즉시 PWM 0이 우선한다.
이 7° 감속이 시작된 경우 MCU는 경사가 해제된 뒤에도 남은 감속 시간을 0 출력으로 완료한다.
평지 복귀는 +3° 이하가 0.5초 지속돼야 한다. 속도 피드백, 손잡이 및 통신 정지는
그대로 우선한다. ToF 정지는 현재 비활성이다. PWM 상한 140은 ROS 명령·브리지·Drive MCU에
...
오르막 FF는 보정 후 -5° 이하이고 오르막 상태가 확정됐을 때 적용한다.
보정 후 -5°에서 +10, -7°에서 +20, -9° 이하에서 +30이다. 기본 FF 60은 명목속도에서의
...
| 조건 | 동작·복귀 |
|---|---|
| 보정 후 pitch ≥ +5°가 0.5초 지속 | 확정 내리막 BRAKE. 목표 0, PWM 0, IN1=IN2=LOW. +3° 이하가 0.5초 지속되면 자동 복귀 |
| 보정 후 pitch ≥ +7°, 확정 전 | MCU TERRAIN_STOP으로 3초 PWM 감속 시작. 내리막 확정 시 BRAKE가 우선 |
...
MPU의 5도 확정 내리막 PWM 0과 7도 조기 감속, 손잡이·통신 보호는 유지한다.
